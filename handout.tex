...
\end{theorem}
\begin{proof}
The $\sigma$-fields $\mathcal{F}_n$ are increasing clearly. Then check the integrability of random variables $H_n^{(c)}$:
...
\begin{proof}
The $\sigma$-fields $\mathcal{F}_n$ are decreasing clearly. Then check the integrability of $U_n$.


Because of the inherent symmetry of the kernel $\psi$ and that samples $X_1, \dotsc, X_k$ are i.i.d, we can write:
\begin{align*}
...
\begin{align*}
    ...
    &=U_n
\end{align*}
since $\mathcal{F}_n \subseteq \mathcal{F}_m$.
\end{proof}
