...
\newcommand{\bd}[2][]{\todo[color=red!40,#1]{BD: #2}}
...
\begin{itemize}
    \item LILY CHANGED THIS. We assume that a sample of random variables $X_1, \dotsc, X_n$ is i.i.d with d.f. $F \in \mathcal{F}\subset \R$. $\theta(F)$ is a functional defined on $\mathcal{F}$. 
    \item P.R. Halmos: 
    \begin{itemize}
    ...
    \end{itemize}
    \item a function $\theta(F)$ is said to admit an unbiased estimator if \begin{align}\label{unbiased}
        E(f_n(X_1, \dotsc, X_n)) = \theta(F)
    \end{align}
    where $f_n(X_1, \dotsc, X_n)$ is a function of the n random variables. 
   \begin{theorem}
    ...
    \item Estimates in this setting only depend on the k observations but we want to restrict to estimators of the form 
    \begin{align}
        \hat{\theta} = f_n(x_1, \dotsc, x_n)
    \end{align}
    such that $f_n$ is a symmetric function that satisfies Equation \ref{unbiased} since the variables are i.i.d.
    
    \item We say that estimates are identical if they agree on some Borel set $E$, which depends on $\mathcal{F}$.
